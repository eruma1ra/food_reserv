    ...
    <header className="bg-background border-b border-border py-4">
      <div className="container px-4 mx-auto flex flex-col md:flex-row items-center justify-between">
        ...
        <div
          ...
          } md:flex flex-col md:flex-row items-center mt-4 md:mt-0 w-full md:w-auto gap-2`}
        >
          <nav className="flex flex-col md:flex-row items-center gap-0.5 md:gap-1 w-full md:w-auto">
            {navItems.map((item) => (
              <Link
                key={item.path}
                to={item.path}
                className={`flex items-center gap-1 text-white hover:text-primary hover:underline hover:decoration-primary hover:decoration-2 px-2 py-2 text-xs md:text-base whitespace-nowrap transition-colors w-full md:w-auto text-center ${
                  isActive(item.path)
